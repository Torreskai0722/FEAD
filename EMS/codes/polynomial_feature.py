...
def check_negative(regr):
	i = 0
	test = []
	for throttle in range(250):
		for rpm in range(500,3000):
			test.append([rpm,throttle])
	n = 5
	poly = PolynomialFeatures(n)# returns: [1, x, x^2, x^3]
	test = poly.fit_transform(test)
	fuel = regr.predict(test)
	for f in fuel:
		# print(f)
		if f < 0:
			i += 1
...
for train_X, train_y, test_X, test_y in kfold_load(X,y):
	...
	# regr = NonnegativeLinear()
	# regr.fit (trX_expanded, train_y)
	

	# Create linear regression object
	# regr = linear_model.LinearRegression()
	regr = linear_model.Lasso(alpha=0.1,positive=True,max_iter=20000)
